// @Description Sub_8010E48_WarKeySJump_Rewritten
// @HookAddress 0x2DEDE0
// @HookString P
// Mode: Thumb
// Made by beco
...
int Sub_8010E48_WarKeySJump_Rewritten() {
    // Vanilla code
		int v1;
		WarioLift_LOBYTE = 0;
	   if ( !Wario_ucJTimer )
	   {
	     if ( Wario_sMvSpeedY > 0 && (!(KeyPressContinuous[0] & 1) || Wario_ucTumiki) )
	       Wario_sMvSpeedY = Wario_ucJTimer;
	     if ( !(KeyPressContinuous[0] & 0x80)
	       && !Sub_8014090_WarHeadCheck(0, 0xFF90, 1)
	       && !Sub_8014090_WarHeadCheck(0, 0xFFC4, 1) )
	     {
	       return JUMP;
	     }
	     ...
	     {
				 v1 = ((Wario_usMukiX ^ 0x30) & KeyPressContinuous[0]) << 16;
	       if ( v1 )
	         Wario_usMukiX ^= 0x30;
	       Wario_sMvSpeedX = 0;
	     }
